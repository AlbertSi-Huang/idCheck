...
namespace LibExchangePlate
{
    ...
    public abstract class PlateExchangeAbstract
    {
        ...
        public virtual void OnBroadcastExchangeInfo(PlateExchangeBakInfo info)
        {
        ...
        private bool CarRecordNeedUpdate(string passTime)
        {
            bool bNeed = true;
            DataRow dr = DatabaseHandle.Single.CarRecords.GetOneData(new List<QueryCondition<ECarCapture>>
            {
                new QueryCondition<ECarCapture>(ECarCapture.passTime,passTime)
            });
            if (dr == null) return bNeed;
        ...
        public virtual int SaveCarRecord(SCarRecord info)
        {
            int nRet = 0;
            if (!CarRecordNeedUpdate(info._passTime)) return nRet;

            DataRow drRecord = DatabaseHandle.Single.CarRecords.NewDataTable().NewRow();
            drRecord[ECarCapture.carColor.ToString()] = info._carColor;
            drRecord[ECarCapture.carNum.ToString()] = info._carNum;
            drRecord[ECarCapture.carType.ToString()] = info._carType;
            drRecord[ECarCapture.idCardNumber.ToString()] = info._idNum;
            drRecord[ECarCapture.imgCar.ToString()] = info._imgCar;
            drRecord[ECarCapture.imgCarNum.ToString()] = info._imgCarNum;
            drRecord[ECarCapture.passTime.ToString()] = info._passTime;

            nRet = DatabaseHandle.Single.CarRecords.InsertData(drRecord);
