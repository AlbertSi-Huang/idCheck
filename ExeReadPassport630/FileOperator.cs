...
namespace ExeReadPassport630
{
    public class FileOperator
    {
        ...
        public bool SaveCardInfo(object obj,string sType,bool bIsAppend = false)
        {
            if (sType == "0")//身份证读卡器设备
            {
            ...
            if (sType == "2")//快证通设备
            {
                ...
                if (!bIsAppend) fm = FileMode.Create;
                using (FileStream fs = File.Open(strPassportInfoFile, fm))
                {
                    using (StreamWriter sw = new StreamWriter(fs))
                    {
                        byte[] data = System.Text.Encoding.Default.GetBytes(strWrite);
