...
namespace TS_IDCheck
{
    class CReadIdCard
    {
        ...
        /// 线程名称
        /// </summary>
        private string ThreadName = "【CReadIdCard】";
        ...
        public CReadIdCard()
        {
            try
            {
                Trace.WriteLine(ThreadName + "实例化 CReadIdCard");
            ...
            {
                Trace.WriteLine(ThreadName + "实例化 CReadIdCard异常:" + ex.Message);
            }
        }
        ...
        private void ReadCard()
        {
            ...
            try
            {
                ...
                Trace.WriteLine(ThreadName + "C卡读卡开始");
                while (true)
                {
                    //如果不需要采集 则直接循环
                    if (!m_isNeedCapture)
                    ...
                    {
                        Trace.WriteLine(ThreadName + "C卡循环读卡线程退出");
                        return;
                    }
                    ...
                    for (int i = 0; i < 9; i++)
                    {
                        ...
                        switch (i)
                        {
                            ...
                            case 2: cardInfo._nation = sb.ToString(); break;
                            case 3: cardInfo._birthday = sb.ToString(); break;
                            case 4:
                                cardInfo._address = sb.ToString();
                                break;
                            case 5: cardInfo._idNum = sb.ToString(); break;
                            case 6: cardInfo._issure = sb.ToString(); break;
        ...
        private void PushCardInfo(SCardInfo info)
        {
            ...
            {
                Trace.Write(ThreadName + "填充身份证信息异常:" + ex.Message);
            }
        }
        ...
        public bool OpenIDCard()
        {
            try
            {
                Trace.Write(ThreadName + "开始打开读卡器...");
                int iRet = InitCommExt();
                if (iRet <= 0)
                {
                    Trace.Write(ThreadName + "读卡器初始化失败 " + iRet);
                    return false;
                }
                m_bInited = true;
                Trace.Write(ThreadName + "读卡器打开完成");
                return true;
            }
            catch (Exception ex)
            {
                Trace.Write(ThreadName + "打开读卡器异常:" + ex.Message);
                return false;
            }
        }
    }
