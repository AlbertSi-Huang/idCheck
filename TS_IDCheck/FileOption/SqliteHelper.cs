...
namespace TS_IDCheck
{
    public class SqliteHelper
    {
        ...
        public void ConnectToDatabase()
        {
            ...
            try
            {
                Trace.WriteLine("Data Source=cache\\TS_Idcheck.db;Version=3;");
                string strCachePath = GetExePath();
                strCachePath += "\\cache";
                if (Directory.Exists(strCachePath) == false)
                {
                    try
                    {
                        Trace.WriteLine("cachePath:" + strCachePath);
                ...
                dbConnection = new SQLiteConnection("Data Source=cache\\TS_Idcheck.db;Version=3;");
                Trace.WriteLine("open sqlite begin");
                dbConnection.Open();
                Trace.WriteLine("open sqlite end");
            }catch(Exception ex)
            {
        ...
        public SQLiteDataReader CreateTable(string tableName, string[] colNames, string[] colTypes)
        {
            ...
            SQLiteDataReader sdr = ExecuteQuery(queryString);
